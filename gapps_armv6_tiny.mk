...
PRODUCT_COPY_FILES += \
	...
	$(shell rm -f $(PRODUCT_OUT)/system/app/GoogleBackupTransport.apk)
	$(shell rm -f $(PRODUCT_OUT)/system/app/GoogleCalendarSyncAdapter.apk)
	$(shell rm -f $(PRODUCT_OUT)/system/app/GoogleContactsSyncAdapter.apk)
	$(shell rm -f $(PRODUCT_OUT)/system/app/GoogleLoginService.apk)
	$(shell rm -f $(PRODUCT_OUT)/system/app/GoogleServicesFramework.apk)
	$(shell rm -f $(PRODUCT_OUT)/system/app/PlayStore.apk)
	$(shell rm -f $(PRODUCT_OUT)/system/app/SetupWizard.ap)
	$(shell rm -f $(PRODUCT_OUT)/system/app/NetworkLocation.apk)
	$(shell rm -f $(PRODUCT_OUT)/system/app/Talk.apk)
	$(shell rm -f $(PRODUCT_OUT)/system/app/ChromeBookmarksSyncAdapter.apk)

	$(shell rm -f $(PRODUCT_OUT)/system/etc/permissions/com.google.android.maps.xml)
	$(shell rm -f $(PRODUCT_OUT)/system/etc/permissions/com.google.android.media.effects.xml)

	$(shell rm -f $(PRODUCT_OUT)/system/framework/com.google.android.maps.jar)
